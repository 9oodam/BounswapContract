...
contract InitialProxy {
    ...
    function poolSharePercent(address tokenA, address tokenB, uint amountADesired, uint amountBDesired) public view returns (uint) {
        ...
        uint sharePercent = (tokenA == token0) ? 
         (amountADesired * 1e18 + amountBDesired * 1e18) / ((reserve0 + amountADesired) + (reserve1 + amountBDesired)) : (amountADesired * 1e18 + amountBDesired * 1e18) / ((reserve0 + amountBDesired) + (reserve1 + amountADesired));
        return sharePercent;
    }
    // 유동성 제거시 퍼센테이지 계산
